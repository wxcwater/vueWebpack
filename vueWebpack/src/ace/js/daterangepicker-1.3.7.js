    ...
    DateRangePicker.prototype = {
        ...
        clickPrev: function (e) {
            ...
            if (cal.hasClass('left')) {
				if(this.format=="YYYY-MM-DD"||this.format=="YYYY-MM-DD HH"||this.format=="YYYY-MM-DD HH:mm"||this.format=="YYYY-MM-DD HH:mm:ss"){
        ...
        buildCalendar: function (month, year, hour, minute, side) {
			...
			if(this.format=="YYYY-MM-DD"||this.format=="YYYY-MM-DD HH"||this.format=="YYYY-MM-DD HH:mm"||this.format=="YYYY-MM-DD HH:mm:ss"){
				...
				var curDate = moment([lastYear, lastMonth, 1, 12, 0]);
				var col, row;
				for (i = 0, col = 0, row = 0; i < 12; i++, col++, curDate = moment(curDate).add('month', 1)) {
					if (i > 0 && col % 4 === 0) {
						col = 0;
						row++;
					}
		...
		renderYearDropdowns: function (selected, minDate, maxDate){
        ...
        renderCalendar: function (calendar, selected, minDate, maxDate) {
			...
			if(this.format=="YYYY-MM"){
				if (this.showDropdowns) {
					dateHtml = this.renderYearDropdowns(calendar[1][1], minDate, maxDate);
				}else{
            ...
            html += '<tbody>';
			var a = 6;
			if(this.format=="YYYY-MM"||this.format=="YYYY"){
				a = 3;
			}
            for (var row = 0; row < a; row++) {
				...
				if(this.format=="YYYY-MM"){
					...
					for (var col = 0; col < 4; col++) {
						var cname = 'available ';
						cname += (calendar[row][col].year() == calendar[1][1].year()) ? '' : 'off';

						if ((minDate && calendar[row][col].isBefore(minDate, 'year')) || (maxDate && calendar[row][col].isAfter(maxDate, 'year'))) {
							cname = ' off disabled ';
						} else if (calendar[row][col].format(this.format) == selected.format(this.format)) {
							cname += ' active ';
							if (calendar[row][col].format(this.format) == this.startDate.format(this.format)) {
								cname += ' start-date ';
							}
							if (calendar[row][col].format(this.format) == this.endDate.format(this.format)) {
								cname += ' end-date ';
							}
						...
						var m = calendar[row][col].month()+1;
						html += '<td style="width:60px;height:35px;" class="' + cname.replace(/\s+/g, ' ').replace(/^\s?(.*?)\s?$/, '$1') + '" data-title="' + title + '">' + m + '月</td>';
					}
					html += '</tr>';
				}else if(this.format=="YYYY"){
